...
from .template_context_vars import DomainVar, ProxyVar
...
_STATIC_DOMAIN_SAMPLE: dict[str, Any] = DomainVar.from_mapping({
    'name': 'example.com',
    'sni': 'example.com',
    'host': 'example.com',
    'server': '203.0.113.1',
...
_STATIC_PROXY_SAMPLE: dict[str, Any] = ProxyVar.from_mapping({
    'tag': 'tls_h2',
    'port': 443,
    'path': 'test-path',
    'server': '203.0.113.1',
...
def _build_static_catalog(lang: str) -> tuple[dict[str, Any], ...]:
    ...
    user_vars = [
        _entry('user', f'user.{field}', field.replace('_', ' ').title(), desc, name=field, access_bracket=f"user['{field}']")
        for field, desc in USER_DICT_KEYS
    ]
    ...
    groups.append({
        ...
        'label': 'Users list',
        'variables': [
            _entry('users', 'users', 'Active users',
                   'List of active users in {% for u in users %} loops (server configs)'),
            _entry('users', 'users', 'Available users',
                   'Active users for custom proxy server inbound templates'),
        ],
    })
    ...
    platform_vars = [
        _entry('platform', f'platform.{field}', field, desc, name=field)
        for field, desc in PLATFORM_VAR_KEYS
    ]
    groups.append({'id': 'platform', 'label': 'Client platform (User-Agent)', 'variables': platform_vars})

    proxy_vars = [
        _entry('proxy', f'proxy.{field}', field, desc, name=field)
        for field, desc in PROXY_VAR_KEYS
    ]
    proxy_vars.extend(
        _entry('proxy', field, field, desc, name=field)
        for field, desc in PROXY_INFO_KEYS
    )
    groups.append({'id': 'proxy', 'label': 'Proxy (current link)', 'variables': proxy_vars})

    cp_vars = [
        _entry('custom_proxy', f'{{{{ {name} }}}}', name, desc, name=name)
        for name, desc in CUSTOM_PROXY_VARS
    ]
...
def build_domain_sample(child_id: int = 0) -> dict[str, Any]:
    ...
    return DomainVar.from_mapping(base).to_dict()
...
def _sample_values(child_id: int) -> dict[str, Any]:
    ...
    active_users = [
        u.to_dict(dump_id=True)
        for u in User.query.filter(User.enable.is_(True)).order_by(User.id).limit(5).all()
    ]
    ...
    for field, _ in DOMAIN_DICT_KEYS:
        access = 'domain.name' if field == 'name' else f'domain.{field}'
        values[access] = _serialize(_STATIC_DOMAIN_SAMPLE.get(field))
    for field, _ in DOMAIN_DERIVED_KEYS:
        values[f'domain.{field}'] = _serialize(_STATIC_DOMAIN_SAMPLE.get(field))
    cert_sample = _STATIC_DOMAIN_SAMPLE.get('cert') or {}
    for field, _ in DOMAIN_CERT_KEYS:
        values[f'domain.cert.{field}'] = _serialize(cert_sample.get(field))
