...
def _parse_friendly_time(value: Any) -> datetime:
    if isinstance(value, datetime):
        return value
    parsed = hutils.convert.json_to_time(value)
    if not isinstance(parsed, datetime):
        raise ValueError(f"Invalid datetime: {value!r}")
    return parsed


def _serialize_friendly_time(value: datetime) -> str | None:
    return hutils.convert.time_to_json(value)


def _parse_friendly_date(value: Any) -> date:
    if isinstance(value, datetime):
        return value.date()
    if isinstance(value, date):
        return value
    parsed = hutils.convert.json_to_date(value)
...
def _serialize_friendly_date(value: date) -> str | None:
    return hutils.convert.date_to_json(value)


FriendlyUUID = Annotated[str, BeforeValidator(_parse_uuid)]
...
class UserSchema(ApiModel):
    ...
    last_online: FriendlyDateTime | None = Field(default=None, description="The last time the user was online, converted to a JSON-friendly format")
    last_modified_time: FriendlyDateTime | None = Field(default=None, description="The last time the user record was modified")
    start_date: FriendlyDate | None = Field(default=None, description="The start date of the user's package, in a JSON-friendly format")
    current_usage_GB: float | None = Field(default=None, description="The current data usage of the user in gigabytes")
    last_reset_time: FriendlyDateTime | None = Field(default=None, description="The last time the user's data usage was reset, in a JSON-friendly format")
    comment: str | None = Field(default=None, description="An optional comment about the user")
    added_by_uuid: FriendlyUUID | None = Field(default=None, description="UUID of the admin who added this user")
...
class PostUserSchema(UserSchema):
    uuid: FriendlyUUID | None = Field(default=None, description="Unique identifier for the user")
...
class PatchUserSchema(ApiModel):
    ...
    last_online: FriendlyDateTime | None = None
    start_date: FriendlyDate | None = None
    current_usage_GB: float | None = None
    last_reset_time: FriendlyDateTime | None = None
    comment: str | None = None
    added_by_uuid: FriendlyUUID | None = None
...
class AdminSchema(ApiModel):
    ...
    uuid: FriendlyUUID | None = Field(default=None, description="The unique identifier for the admin")
    ...
    parent_admin_uuid: FriendlyUUID | None = Field(default=None, description="The unique identifier for the parent admin")
...
class PatchAdminSchema(ApiModel):
    name: str | None = None
    comment: str | None = None
    uuid: FriendlyUUID | None = None
    mode: AdminMode | None = None
    can_add_admin: bool | None = None
    parent_admin_uuid: FriendlyUUID | None = None
    telegram_id: int | None = None
    lang: Lang | None = None
