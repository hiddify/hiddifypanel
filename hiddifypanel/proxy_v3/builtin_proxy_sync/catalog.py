...
from hiddifypanel.models.proxy_base_config import BUILTIN_BASE_CONFIGS
...
def _load_content(slug: str, path: Path) -> str:
    return load_template_slug(slug, normalize=True)
...
def discover_base_configs() -> tuple[BuiltinBaseConfigRecord, ...]:
    ...
    for spec in BUILTIN_BASE_CONFIGS:
        ...
        try:
            content = catalog_base_content(side_val, core)
        except FileNotFoundError:
            logger.warning("Skipping builtin base config {}/{}: missing proxy_templates/{}/{}/base.j2", side_val, core, core, side_val)
            continue
        records.append(
            BuiltinBaseConfigRecord(
                core=core,
                side=side_val,
