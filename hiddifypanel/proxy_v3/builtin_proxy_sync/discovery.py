...
_BASE_CONFIG_RE = re.compile(
    r'^([^/]+)/(client|server)/base\.j2$',
)
...
def iter_template_files() -> Iterator[tuple[str, Path]]:
    """Discover fragment templates: .pj2 files and non-base .j2 files (auto-sync on new files)."""
    ...
    for path in sorted(TEMPLATES_ROOT.rglob('*')):
        ...
        if path.suffix == '.pj2':
            ...
            yield slug, path
        elif path.suffix == '.j2' and not is_base_config_path(rel):
            slug = str(path.relative_to(TEMPLATES_ROOT).with_suffix('')).replace('\\', '/')
            yield slug, path
