...
from hiddifypanel.hutils.proxy.custom_proxy_ports import (
...
class ProxyTemplate(db.Model):  # type: ignore
    ...
    __table_args__ = (
        UniqueConstraint('child_id', 'slug', name='uq_proxy_template_child_slug'),
    )
    ...
    def effective_content(self) -> str:
        from hiddifypanel.panel.template_catalog.builtin_sync import effective_template_content
    ...
    def add_or_update(cls, child_id: int = 0, commit: bool = True, **data) -> 'ProxyTemplate':
        ...
        if template_id:
            db_tpl = cls.query.filter(cls.id == template_id, cls.child_id == child_id).first()
        if not db_tpl:
        ...
        if db_tpl.is_builtin:
            ...
            from hiddifypanel.panel.template_catalog.builtin_sync import apply_builtin_override_template
            if 'name' in data:
            ...
            if 'content' in data:
                ...
                if not db_tpl.builtin_override and new_content != (db_tpl.builtin_content or ''):
...
class CustomProxyClientCore(db.Model):  # type: ignore
    ...
    __table_args__ = (
        UniqueConstraint('custom_proxy_id', 'core', 'version', name='uq_custom_proxy_client_core'),
    )
    ...
    do_base64_after = Column(Boolean, default=False, nullable=False)
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.core == ClientCore.sublink:
            payload['do_base64_after'] = bool(self.do_base64_after)
        return payload
...
class CustomProxy(db.Model):  # type: ignore
    ...
    __table_args__ = (
        UniqueConstraint('child_id', 'slug', name='uq_custom_proxy_child_slug'),
    )
    ...
    def effective_server_config_text(self) -> str:
        if self.is_builtin and not self.server_override:
            return self.builtin_server_config or self.server_config or ''
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        builtin_client_configs = [
            {
                ...
                'outbounds_template': row.builtin_outbounds_template or '',
                'do_base64_after': bool(row.do_base64_after),
                'template_slugs': list(row.template_slugs or []),
            }
        ...
        return {
            ...
            'server_config': {
                ...
                'template_slugs': list(self.server_template_slugs or []),
            ...
            'builtin_server_config': self.builtin_server_config or '',
            'builtin_client_config': {'core_configs': builtin_client_configs},
            'server_override': bool(self.server_override),
            'client_override': any(row.override for row in self.client_cores),
    ...
    def add_or_update(cls, child_id: int = 0, commit: bool = True, **data) -> 'CustomProxy':
        ...
        if dbproxy.is_builtin:
            ...
            from hiddifypanel.panel.template_catalog.builtin_sync import apply_custom_proxy_general, apply_server_override
            if 'name' in data:
            ...
            if 'server_override' in data:
                apply_server_override(dbproxy, override=bool(data['server_override']))
            apply_server = bool(dbproxy.server_override or dbproxy.id is None)
            if 'server_config' in data and apply_server:
                ...
                if 'template_slugs' in payload:
                    dbproxy.server_template_slugs = list(payload.get('template_slugs') or [])
            ...
            general_keys = ('alpns', 'download_alpns', 'custom_path', 'domain_modes')
            ...
            if 'mode' in data and 'domain_modes' not in data:
        ...
        if 'download_alpns' in data:
            dbproxy.download_alpns = [
                str(v).strip() for v in (data.get('download_alpns') or []) if str(v).strip()
            ]
        if 'tags' in data:
            dbproxy.tags = list(data.get('tags') or [])
        if 'domain_modes' in data:
        ...
        if 'client_config' in data:
            ...
            for item in core_configs:
                ...
                dbproxy.client_cores.append(CustomProxyClientCore(
                    core=core,
    ...
    def _sync_builtin_client_overrides(cls, dbproxy: 'CustomProxy', client_config: dict[str, Any]) -> None:
        for item in client_config.get('core_configs') or []:
            ...
            if not row:
                row = CustomProxyClientCore(core=core, version=str(item.get('version') or ''))
            ...
            if 'do_base64_after' in item:
                row.do_base64_after = bool(item.get('do_base64_after'))
            if 'template_slugs' in item:
                row.template_slugs = list(item.get('template_slugs') or [])
...
def seed_default_proxy_shells(child_id: int = 0) -> None:
    from hiddifypanel.panel.template_catalog.template_defaults import (
...
def seed_proxy_templates(child_id: int = 0) -> None:
    from hiddifypanel.panel.template_catalog.builtin_sync import sync_builtin_template
    from hiddifypanel.panel.template_catalog.builtin_templates import build_builtin_templates, get_builtin_templates

    build_builtin_templates.cache_clear()
    seed_default_proxy_shells(child_id)
    ...
    for tpl in templates:
        ...
        if row:
            ...
            if sync_builtin_template(row, tpl):
                updated += 1
            continue
        catalog_content = tpl.get('content') or ''
        db.session.add(ProxyTemplate(
            child_id=child_id,
            slug=tpl['slug'],
            core=_parse_template_core(tpl['core']),
        ...
        added += 1
    stale = ProxyTemplate.query.filter(
        ProxyTemplate.child_id == child_id,
        ProxyTemplate.is_builtin.is_(True),
    ).filter(~ProxyTemplate.slug.in_(disk_slugs)).all()
    for row in stale:
    ...
    logger.info(
        'Proxy templates: child_id={} added={} updated={} removed={} total={}',
        child_id, added, updated, removed, len(templates),
    )

    from hiddifypanel.panel.template_catalog.custom_proxy_presets import seed_custom_proxy_presets
